...
var gameState = {
  ...
  elements: {
    //Select all the pokemon and put into a variable
    pokemonsEl: document
      .querySelector(".select-screen")
      .querySelectorAll(".character"),
    //changing the screen once pokemon is clicked
  ...
  init: function() {
    //loop through variable and have it display name of the pokemon if clicked
    var i = 0;
    while (i < gameState.elements.pokemonsEl.length) {
      ...
      gameState.elements.pokemonsEl[i].onclick = function() {
        ...
        //elements for the images on battle screen
        ...
        //changing the pokemon pic to the correct choosen one
  ...
  attackMove: function(attack, level, stack, critical, enemy) {
    var attackAmount = attack * level * (stack * critical);
    enemy.health = enemy.health - attackAmount;

    //bringing down health visually
    ...
    gameState.checkWinner(enemy.health);
  },

  checkWinner: function(enemy) {
    if (gameState.checkWinner <= 0) {
  ...
  //random number function for the rival cpu
  randomNumber: function(min, max) {
    return Math.floor(Math.random() * (max - min)) * min;
  },

  //rival pokemon function
  cpuPick: function() {
